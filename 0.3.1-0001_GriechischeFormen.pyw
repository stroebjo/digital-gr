...
Klasse die es ermoeglicht eine bestimmte Form vom Programm bilden zu lassen.
    ...
    def __init__(self, parent):
        ...
        self.vonButton = wx.Button(
            self, -1, 'Bilde die eingestellte Form', size=(180, 23))
    ...
    def formen(self, event):
        if len(self.vonVerb.GetLabel().replace(" ", "")) > 1:
            ...
            try:
                ...
                error = wx.MessageDialog(None, "Es trat ein Fehler auf!",
                                         "Info", wx.ICON_ERROR)
                error.ShowModal()
        else:
    ...
    def getThisForm(self, event):
        if len(self.vonVerb.GetLabel().replace(" ", "")) > 1:
            ...
            # Futur des Perfekt ab
            if modusSelection == 2 and genusSelection == 0 and temporaSelection in [
                    1, 5, 6
            ] and genusSelection == 0:
                wx.MessageBox("Den "+q.solve_tempora[temporaSelection]+\
                     " gibt es im Optativ Aktiv nicht!", "Info")
                return

            # Wenn der Optativ Medium / Passiv  gewaehlt ist, breche bei Impf, Plusqam ab
            if modusSelection == 2 and genusSelection == 1 and temporaSelection in [
                    1, 5
            ] and genusSelection in [1, 2]:
                wx.MessageBox("Den "+q.solve_tempora[temporaSelection]+\
            ...
            if modusSelection == 3 and temporaSelection in [
                    1, 2, 4, 5, 6
            ] and genusSelection == 0:
                wx.MessageBox("Den "+q.solve_tempora[temporaSelection]+\
            ...
            if modusSelection == 3 and personSelection in [0, 3]:
                wx.MessageBox(
                    "Im Imperativ gibt es die 1. Person Singular / Plural nicht!",
                    "Info")
                return

            form = newForm.VerbForm().generateForm(
                fontflag=self.fontmodus,
    ...
    def AddRadios(self):
        ...
        self.genus = wx.RadioBox(
            ...
            style=wx.RA_SPECIFY_ROWS | wx.NO_BORDER | wx.EXPAND)
...
class BuildVerb(wx.Panel):
    def __init__(self, parent):
        ...
        self.values, self.form, self.verb = newForm.VerbForm().generateForm(
            self.fontmodus)
        ...
        self.target = wx.StaticText(self, -1,
               ...
               self.verb)
        self.target.SetFont(
            wx.Font(10, wx.NORMAL, wx.NORMAL, wx.NORMAL, False))
        ...
        inputGrid.Add(
            self.check, (0, 1), flag=wx.TOP | wx.ALIGN_LEFT, border=4)
        sizer_input.Add(inputGrid)
        sizer.Add(sizer_input, 0,
                  wx.ALIGN_LEFT | wx.EXPAND | wx.LEFT | wx.BOTTOM | wx.RIGHT,
                  9)
...
class DetermineVerb(wx.Panel):
    def __init__(self, parent):
        ...
        self.symbolFont = wx.Font(13, wx.NORMAL, wx.NORMAL, wx.NORMAL, False,
                                  'Symbol')
        self.griechFont = wx.Font(16, wx.NORMAL, wx.NORMAL, wx.NORMAL, False,
                                  'Griech2')
        ...
        self.output = wx.TextCtrl(
            self, -1, size=(250, 28), style=wx.TE_READONLY)
        ...
        outputGrid.Add(
            self.generateNew, (0, 1), flag=wx.TOP | wx.ALIGN_LEFT, border=4)

        sizer_output.Add(outputGrid, flag=wx.EXPAND)

        sizer.Add(sizer_output, 0,
                  wx.ALIGN_LEFT | wx.EXPAND | wx.LEFT | wx.RIGHT, 9)
        ...
        self.solution = wx.StaticText(
            panel_eval, -1, '', pos=(100, 8), size=(280, 20))
    ...
    def OnEval(self, event):
        self.input = [
            self.modus.GetSelection(),
            self.tempora.GetSelection(),
            self.genus.GetSelection(),
            self.person.GetSelection()
        ]
...
#===============================================================================
    ...
    def OnSolve(self, event):
        ...
        self.solution.SetFont(
            wx.Font(9, wx.NORMAL, wx.NORMAL, wx.NORMAL, False))
...
class LearnGUI(wx.Frame):
    def __init__(self, parent=None, id=-1, title='NoName'):
        ...
        self.Bind(
            wx.EVT_MENU,
            self.exportEndungenDialog,
            id=self.exportEndungen.GetId())
...
Fuegt die Menueleiste hinzu.
        ...
        self.determineform = datei.Append(
            ID_DTERMINE,
            'Verben bestimmen',
            'Blendet den Dialog zum Bestimmen von Verben ein',
            kind=wx.ITEM_RADIO)
        self.buildform = datei.Append(
            ID_BUILD,
            'Verben bilden',
        ...
        ## export ----------------------------------------------------------------------
        export = wx.Menu()
        self.exportEndungen = export.Append(
            wx.ID_ANY, 'Endungen',
            'Exportiert alle Endungen in eine HTML Datei')
        ...
        ## PREFERENCES =================================================================
        preferences = wx.Menu()
        self.verb = preferences.Append(wx.ID_ANY, "&Verben",
                                       "Verben hinzufuegen / entfernen")

        ## font ------------------------------------------------------------------------
        font = wx.Menu()
        self.symbolItem = font.Append(
            wx.ID_ANY,
            'S&ymbol',
            'Es werden keine Akzente dargestellt',
            kind=wx.ITEM_RADIO)
        self.griechItem = font.Append(
            wx.ID_ANY,
            '&Griech2',
