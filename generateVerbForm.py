...
class VerbForm(object):
    ...
    def generateForm(self,
        ...
        if (modus != 666) and (tempora != 666) and (genus != 666) and (person
                                                                       != 666):
            self.modus = modus
        ...
        self.endung = q.endungen[self.modus][self.tempora][self.genus][
            self.person]
        ...
        if verb != 'w' or 'omai':
            if self.fontflag == 1:
                if noAmb == 0:
                    return (self.findAmbiguities(self.form, verb),
                            self.makeGriech2(self.form),
                            self.makeGriech2(verb))
                else:
                    return ([self.vals], self.makeGriech2(self.form),
                            self.makeGriech2(verb))
            else:
                if noAmb == 0:
                    return (self.findAmbiguities(self.form, verb), self.form,
                            verb)
                else:
...
ruft dafuer entsprechende Funktionen auf, die dies dann durchfuehren.
    ...
    def checkMuta(self):
        ...
        if stammauslaut in ['tt'] or stammauslaut[-1] in ['g', 'k', 'c'
                                                          ]:  # c = Chie (x)
            status = 1
            self.setKStaemme()  # Gutteral- oder K-Stamme
        elif stammauslaut in ['pt'] or stammauslaut[-1] in ['p', 'b', 'j'
                                                            ]:  # j = Phie
            status = 1
    ...
    def setKStaemme(self):
        ...
        if endung[:2] in ['sJ']:
            ...
            self.form = self.form + self.starkesPerfekt(
                self.TempusSign) + self.endung
    ...
    def setAContracta(self):
        ...
        if endung[:2] in ['oi']:
            ...
            self.form = form + 'w' + endung[2:]
                ...
                'ei', 'h#'
        ]:  # the 'h#' if  self.form is generated with Grich2 (not implemented yet)
            if self.fontflag != 1:
    ...
    def makeGriech2(self, word):
        ...
        if word.find(' (iotasub.!)') != -1:
            ...
            word = word.replace('h', '#h',
                                1)  # Das '#' (iotasub.) muss vor dem h stehen
...
Waehlt ein Verb aus.
        """
        return self.killErstePerson(
            self.verbalstock[rand.randint(0, len(self.verbalstock) - 1)])
...
##            person = rand.randint(0, 7)
    ...
    def getAugment(self):
        ...
        augment = ''
        if self.modus == 0 and self.tempora in [
                1, 3
        ]:  # tempora == Ind. Impf or Ind. Aor.?
            if self.setSpecialAugment(
            ) == 0:  # Fals kein unregelmaessiges Augment vorlieht
    ...
    def setReduplikation(self, modus='perf'):
        ...
        first = self.form[
            0]  # get first char of the form to determine the reduplikation
        form = self.form[1:]
        redu = ''
    ...
    def getTempusSign(self):
        ...
        if (self.tempora in [2, 3] and self.genus in [0, 1]):  # Anmerk. [1]
            tempusSign += q.tempuszeichen[
                0]  # 's' als q.tempuszeichen hinzufuegen
